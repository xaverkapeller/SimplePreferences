...
public class PreferencesCompiler extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, final RoundEnvironment roundEnv) {
        ...
        annotations.stream().flatMap(element -> roundEnv.getElementsAnnotatedWith(element).stream())
                .filter(TypeElement.class::isInstance)
                .map(TypeElement.class::cast)
                .forEach(this::buildClass);

        return false;
    }

    private void buildClass(TypeElement element) {
        final Type interfaceType = Types.create(element);
        ...
        try {
            ...
            final Constructor constructor = builder.addConstructor(EnumSet.of(Modifier.PUBLIC), new ExecutableBuilder() {
                ...
                public void writeBody(CodeBlock code, VariableGenerator generator) {
                    ...
                    code.append(fieldContext.set(paramContext)).append(";\n");
                }
            });
