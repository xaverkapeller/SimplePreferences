 ...
 */
class LongGetterExecutableBuilderImpl extends ExecutableBuilder {
    ...
    public LongGetterExecutableBuilderImpl(ProcessingEnvironment processingEnvironment, ExecutableElement method, Field fieldPreferences, String key) {
    ...
    protected void write(Block block) {
        if (Utils.hasOneAnnotationOf(mMethod, LONG_BLACKLIST)) {
        ...
        block.append("return ").append(mFieldPreferences).append(".getLong(").append(Values.of(mKey)).append(", ").append(defaultValue).append(");");
    }

    @SuppressWarnings("ConstantConditions")
    private CodeElement getDefaultElement() {
        if (Utils.hasAnnotation(mMethod, SimplePreferencesAnnotations.ANNOTATION_DEFAULT_LONG_VALUE)) {
            final long defaultValue = (long) Utils.getAnnotationValue(mMethod, SimplePreferencesAnnotations.ANNOTATION_DEFAULT_LONG_VALUE, "value").getValue();
