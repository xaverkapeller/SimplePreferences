 ...
 */
class IntegerGetterExecutableBuilderImpl extends ExecutableBuilder {
    ...
    public IntegerGetterExecutableBuilderImpl(ProcessingEnvironment processingEnvironment, ExecutableElement method, Field fieldPreferences, String key) {
    ...
    protected void write(Block block) {
        if (Utils.hasOneAnnotationOf(mMethod, INTEGER_BLACKLIST)) {
        ...
        block.append("return ").append(mFieldPreferences).append(".getInt(").append(Values.of(mKey)).append(", ").append(defaultValue).append(");");
    }

    @SuppressWarnings("ConstantConditions")
    private CodeElement getDefaultElement() {
        if (Utils.hasAnnotation(mMethod, SimplePreferencesAnnotations.ANNOTATION_DEFAULT_INTEGER_VALUE)) {
            final int defaultValue = (int) Utils.getAnnotationValue(mMethod, SimplePreferencesAnnotations.ANNOTATION_DEFAULT_INTEGER_VALUE, "value").getValue();
